...
static int _set_led(int pin_number ,int on)
{
	resource_write_led(pin_number, on);

	return 0;
}


static Eina_Bool _tracking_to_led(void *user_data)
{
	...
	int value = 0;
	int pinnb[2]={0,};
	int defaultpin[2]={130,46};
	const char* path="/opt/usr/home/owner/apps_rw/org.tizen.smart-surveillance-camera/shared/data/lhy.txt";
/*
...
*/
	result=read_tracking_data(path, &value, &pinnb[0], &pinnb[1]);


	if(result==-1)
	{
		resource_write_led(46, 0);
		resource_write_led(130, 0);

		resource_write_led(46, 1);
		resource_write_led(130, 1);
	...
	for (i = 0; i < 2; i++) {
		//if (pinnb[i] == 0)
			//resource_write_led(defaultpin[i], 0);
	//	else
			resource_write_led(pinnb[i], 1);
	}
...
static void __thread_end_cb(void *data, Ecore_Thread *th)
{
	...
	_I("__thread_end_cb function called!!");
